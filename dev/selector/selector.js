...
Selector = {
	 ...
	 */
	all: function (selector, context, result) {
		...
		while (selector) {
			...
			if (selector) {
				...
				if (match = /^\s*,\s*/.exec(selector)) {
					ap.push.apply(result, Selector.all(RegExp.rightContext, context, []));
					break;
				}
	...
	filter: function (selector, result) {
		...
		while (result.length && (match = /^([#\.:]|\[\s*)((?:[-\w]|[^\x00-\xa0]|\\.)+)/.exec(selector))) {
			...
			if (!filterFn) {
				...
				filterFn += 'if(t)r[n++]=e;}Array.prototype.splice.call(r,n);';

				filterFn = new Function('r', 'v', filterFn);
