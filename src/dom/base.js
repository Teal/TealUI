...
var Dom = (function () {
	...
	Dom.getText = function (node) {
		assert.isNode(elem, "Dom.getText(node, name): {node} ~");
	...
	Dom.toggleClass = function (elem, className, state) {
		Dom[state == undefined ? Dom.hasClass(elem, className) : !state ? 'removeClass' : 'addClass'](elem);
	};
	...
	Dom.remove = function (node) {
		node.parentNode && node.parentNode.removeChild(elem);
	};
