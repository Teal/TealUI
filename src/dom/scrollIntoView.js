 ...
 */
Element.prototype.scrollIntoViewIfNeeded = function (alignCenter, duration, scrollContainer) {

    // 直接调用原生 API。
    if (this._scrollIntoViewIfNeeded && !duration && !scrollContainer) {
        return this._scrollIntoViewIfNeeded(alignCenter);
    }

    var elem = this,
        containerRect = (scrollContainer = scrollContainer || elem.getScrollParent()).getRect(),
        currentRect = elem.getRect(),
        deltaY = currentRect.top - containerRect.top,
        deltaX = currentRect.left - containerRect.left,
        currentScroll = scrollContainer.getScroll(),
    ...
    } else if (deltaY > containerRect.height) {
    ...
    } else if (deltaX > containerRect.width) {
    ...
    // 更新并移动位置。
    (currentScroll.top != null || currentScroll.left != null) && scrollContainer.scrollTo(currentScroll.left, currentScroll.top, duration);
