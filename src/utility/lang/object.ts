 ...
 * @see https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Object/assign
 */
export function assign$simple<T>(target: T, source: any) {
    for (const key in source) {
        target[key] = source[key];
    }
    return target;
 ...
 * @see https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Object/create
 */
export function create$simple<T>(obj: T): T {
 ...
 */
export function map<T, R>(iterable: ArrayLike<T> | { [key: string]: T }, callback: (value: T, index: number | string, target: R) => boolean, scope?: any): R[] | { [key: string]: R } {
    let result;
    if (iterable && typeof (iterable as ArrayLike<T>).length === "number") {
        result = [];
        const length = (iterable as ArrayLike<T>).length;
 ...
 * @example export function isEmpty(null) // true
 * @example export function isEmpty(undefined) // true
 * @example export function isEmpty("") // true
 * @example export function isEmpty(" ") // false
 * @example export function isEmpty([]) // true
 * @example export function isEmpty({}) // false
 */
 ...
 * @example export function isObject({}) // true
 * @example export function isObject(null) // false
 */
 ...
 * @example export function clone({a: 3, b: [5]}) // {a: 3, b: [5]}
 */
 ...
 * @example export function deepClone({a: 3, b: [5]}) // {a: 3, b: [5]}
 */
    ...
    if (obj && typeof obj === 'object') {
        if (obj instanceof Array) {
            ...
            for (let i = 0; i < obj.length; i++) {
                newObj[i] = export function deepClone(obj[i]);
            }
            obj = newObj;
        } else if (obj instanceof Date) {
            obj = new Date(+obj);
 ...
 * @example export function keyOf({a:1, b:1}, 1) // "a"
 */
 ...
 * @example export function size({a: 1, b: 2}) // 2
 * @example export function size([0, 1]) // 2
 */
export function size(obj: any) {
    let result = 0;
    for (const key in obj) {
        if (export function prototype.hasOwnProperty.call(obj, key)) {
            result++;
        }
    }
    return result;
 ...
 * @example export function get({a: {b: 1}}, "a.b") // 1
 */
export function get<T>(obj: T, prop: string) {
 ...
 * @param value 要设置的值。
 * @returns 返回 *obj*。
 * @example export function set({}, "a[1].b", 2) // {a:[undefined, {b: 2}]}
 */
 ...
 * @returns 返回 *obj*。
 * @example export function insertBefore({a:1}, 'a', 'b', 2) // {b:2, a: 1}
 */
 ...
 * @example export function pick(undefined, null, 1) // 1
 */
 ...
 * @param keys 要获取的键列表。
 * @returns 返回新对象。
 * @example export function subset({a: 1, b: 2}, ['a']) // {a: 1}
 */
export function subset(obj: any, keys: string[]) {
    let result = {};
 ...
 * @param objY 要比较的第二个对象。
 * @returns 返回一个对象。
 * @example export function diff({a:1, c: 1}, {b:1, c: 2}) // {left: ["a"], right: ["b"], both: ["c"]}
 */
