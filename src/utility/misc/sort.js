...
function bubbleSort(iteratable, compareFn, start, end) {
    ...
    window.console && console.assert(compareFn instanceof Function, "bubbleSort(iteratable, compareFn: 必须是函数, [start], [end])");
    start = start || 0;
...
function shellSort(iteratable, compareFn, start, end) {
    compareFn = compareFn || function (x, y) { return x < y; };
    window.console && console.assert(compareFn instanceof Function, "shellSort(iteratable, compareFn: 必须是函数, [start], [end])");
    start = start || 0;
    end = end || iteratable.length;
...
function quickSort(iteratable, compareFn, start, end) {
    window.console && console.assert(!compareFn || compareFn instanceof Function, "quickSort(iteratable, compareFn: 必须是函数, [start], [end])");
