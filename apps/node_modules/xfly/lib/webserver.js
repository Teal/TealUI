		...
		if(!server) {
			...
			server.on('error', function(e){
				...
				if (e.code == 'EADDRINUSE') {
					me.error('[Error]Cannot create server on port ' + application.port + (application.address && application.address !== '0.0.0.0' ? ' of ' + application.address : ''));
				} else {
					me.error(e);
				}
			});
			...
			server.on('listening', function(){
				...
				me.log("[INFO]Server running at " + (defaultApp || this.applications[0]).rootUrl);
				me.log("[NOTE]Close this window to stop server.");
			});
			...
			server.on('close', function(){
				...
				me.log("[info]Server stopped at " + (defaultApp || this.applications[0]).rootUrl);
			});
