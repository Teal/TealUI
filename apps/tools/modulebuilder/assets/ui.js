...
var UI = {
		...
		switch (value) {
			case 1:
				...
				if (UI.usingXFly) {
					...
					UI.usingXFly = false;
					Ajax.jsonp(Demo.Configs.serverBaseUrl + Demo.Configs.apps + "/tools/modulebuilder/server/api.njs?action=recentbuildfiles", function (data) {
						UI.usingXFly = true;
	...
	_showBuildFileListHtml: function (id, title, data, fromOpenList) {
		...
		if (data && data.length) {
			...
			for (var i = 0; i < data.length; i++) {
				r.push('<div class="line" onmouseover="this.className += \' line-hover\'" onmouseout="this.className = this.className.replace(\' line-hover\', \'\');"><nav class="demo demo-toolbar"><a href="javascript://打开此打包方案" onclick="UI.openBuildFile(\'' + data[i] + '\',' + fromOpenList + ');return false;">打开</a> | <a class="demo-viewsource-toggle" href="javascript://删除此打包方案" onclick="UI.deleteBuildFile(\'' + data[i] + '\',' + fromOpenList + '); return false;">删除</a> | <a href="javascript://复制此打包方案" onclick="UI.copyBuildFile(\'' + data[i] + '\',' + fromOpenList + ');return false;">复制</a> |  <a class="demo-viewsource-toggle" href="javascript://根据此打包方案重新生成" onclick="UI.buildBuildFile(\'' + data[i] + '\',' + fromOpenList + '); return false;">生成</a></nav><a class="link" href="javascript://打开此打包方案" onclick="UI.openBuildFile(\'' + data[i] + '\',' + fromOpenList + ');return false;">' + (fromOpenList ? data[i] : Path.basename(data[i], ".build.js")) + '</a></div>');
			}
	...
	_jsonpApi: function (action, data, callback) {
		Ajax.jsonp(Demo.Configs.serverBaseUrl + Demo.Configs.apps + "/tools/modulebuilder/server/api.njs?action=" + action, data, callback, UI._alertOpenXFlyError);
	},
	...
	_checkValue: function (id, callback) {
		...
		if (id.value) {
			...
			Dom.addClass(Dom.get('step2_url'), 'ui-textbox-error');
		}
	},
	...
	viewSource: function (node, module) {
		...
		UI.updateBuildFile();
		UI._getSource(UI.currentBuildFile, module, function (data) {
	...
	viewRequires: function (node, module) {
		...
		UI.updateBuildFile();
		UI._getRequires(UI.currentBuildFile, module, function (data) {
	...
	// 获取某个的模块源码。
	_getSource: function (file, modulePath, callback) {
		var mb = new ModuleBuilder(file);
		var r = {};
		var type = mb.getModuleType(modulePath);
		if (type) {
		...
		function checkFile(modulePath, callback) {
			var url = mb.getFullPath(modulePath, me.file.path);
			mb.loadContent(url, function (error, content) {
				if (!error) {
					r[modulePath] = url;
				}
	...
	// 获取某个的模块引用项。
	_getRequires: function (file, modulePath, callback) {
		var mb = new ModuleBuilder(file);
		var r = {};
		var type = mb.getModuleType(modulePath);
		if (type) {
		...
		function checkFile(modulePath, callback) {
			var url = mb.getFullPath(modulePath, mb.file.path);
			mb.loadContent(url, function (error, content) {
				if (!error) {
					var type = mb.getModuleType(modulePath);
					var rq;
					...
					if (type == ".js") {
						rq = mb.resolveJsRequires(content, modulePath, url);
					} else if (type == ".css") {
						rq = mb.resolveCssRequires(content, modulePath, url);
					}
