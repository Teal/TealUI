        ...
        initSourceCode: function () {
            ...
            Doc.Dom.each('.doc > pre, .doc-demo', function (node) {
                ...
                if (node.tagName === 'PRE') {
                    ...
                    node.className = ''; // 避免影响样式。
                } else {
                    content = node.innerHTML;
                    language = 'html';
                }

                language = language || (pre && (/\bdoc-code-(\w+)\b/.exec(pre.className) || [])[1]) || Doc.SyntaxHighligher.guessLanguage(content);
                if (!pre) {
                ...
                if (language == 'js' || ( language == 'html' && node.tagName !== 'SCRIPT')) {
