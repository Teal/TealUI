import { getScroll, setScroll, getRect, Size, getStyle, Point } from "ux/dom";
 ...
 */
export function scrollSize(elem: HTMLElement | Document) {
    return {
        width: elem.nodeType === 9 ? Math.max((elem as Document).documentElement.scrollWidth || 0, (elem as Document).body.scrollWidth || 0, (elem as HTMLElement).clientWidth || 0) : (elem as HTMLElement).scrollWidth,
        height: elem.nodeType === 9 ? Math.max((elem as Document).documentElement.scrollHeight || 0, (elem as Document).body.scrollHeight || 0, (elem as HTMLElement).clientHeight || 0) : (elem as HTMLElement).scrollHeight
    } as Size;
}
 ...
 */
export function scrollShow(elem: HTMLElement | Document, callback: () => void, once = true, scrollable: HTMLElement | Document = scrollParent(elem), padding = 0) {
...
export function scrollParent(elem: HTMLElement | Document) {
    if (elem.nodeType !== 9) {
        while ((elem = elem.parentNode as HTMLElement) && elem.nodeType === 1 && !/^(?:auto|scroll)$/.test(getStyle(elem, "overflow")));
    }
