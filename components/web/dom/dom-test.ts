...
export function parseTest() {
    ...
    assert.strictEqual((dom.parse("<input type='text' value='TEST' />") as HTMLInputElement).value, "TEST", "默认值");
    assert.strictEqual((dom.parse("<div/>") as HTMLElement).tagName, 'DIV', "确保空白被删除");
    ...
    assert.ok(!(dom.parse("<option>test</option>") as HTMLOptionElement).selected, "Make sure that options are not auto-selected");
    ...
    assert.strictEqual((dom.parse("<span/>", dom.parse("<div/>").ownerDocument) as HTMLElement).tagName, 'SPAN', "Verify a span created with a div context works");
}
...
export function closestTest() {
    ...
    assert.strictEqual(dom.closest(dom.find(dom.parse("<div><p></p></div>") as HTMLElement, "p")!, "table"), null, "Make sure disconnected closest work.");
...
export function appendTest() {
    ...
    assert.ok((dom.find(document.getElementById("form")!, "input[name=radiotest]") as HTMLInputElement).checked, "Append checked radio");
    ...
    assert.ok((dom.find(document.getElementById("form")!, "input[name=radiotest]") as HTMLInputElement).checked, "Append HTML5-formated checked radio");
    ...
    var colgroup = dom.append(document.getElementById("table")!, "<colgroup></colgroup>") as HTMLElement;
...
export function cloneTest() {
    ...
    for (var i = 0; i < cloneTags.length; i++) {
        var j = dom.parse(cloneTags[i]) as HTMLElement;
    ...
    var divEvt = dom.parse("<div><ul><li>test</li></ul></div>") as HTMLElement;
    ...
    div = dom.parse("<div/>") as HTMLElement;
    ...
    var element = dom.parse("<select><option>Foo</option><option selected>Bar</option></select>") as HTMLElement;
    ...
    element = dom.parse("<input type='checkbox' value='foo'>") as HTMLElement;
    dom.setAttr(element, "checked", "checked");
    clone = dom.clone(element);

    assert.strictEqual((clone as HTMLInputElement).defaultValue, "foo", "Checked input defaultValue cloned correctly");
    ...
    element = dom.parse("<input type='text' value='foo'>") as HTMLElement;
    clone = dom.clone(element);
    assert.strictEqual((clone as HTMLInputElement).defaultValue, "foo", "Text input defaultValue cloned correctly");

    element = dom.parse("<textarea>foo</textarea>") as HTMLElement;
    clone = dom.clone(element);
    assert.strictEqual((clone as HTMLInputElement).defaultValue, "foo", "Textarea defaultValue cloned correctly");

    // clone(multiple selected options
    element = dom.parse("<select><option>Foo</option><option selected>Bar</option><option selected>Baz</option></select>") as HTMLElement;
    clone = dom.clone(element);
...
export function getAttrTest() {
    ...
    assert.strictEqual(dom.getAttr(dom.parse("<div value='t'></div>") as HTMLElement, "value"), "t", "Check setting custom attr named 'value' on a div");
    ...
    var $img = document.body.appendChild(dom.parse("<img style='display:none' width='215' height='53' src='../../assets/resources/userface.png' />")) as HTMLElement;
    ...
    var $a = document.getElementById("qunit-fixture")!.appendChild(dom.parse("<a href='#' onclick='return 1'>Click</a>")) as HTMLElement;
    assert.strictEqual(dom.getAttr($a, "onclick")(), 1, "Retrieve ^on attribute without anonymous function wrapper.");

    assert.strictEqual(dom.getAttr(dom.parse("<div/>") as HTMLElement, "doesntexist"), null, "Make sure null is returned when no attribute is found.");
    assert.strictEqual(dom.getAttr(dom.parse("<div/>") as HTMLElement, "title"), "", "Make sure null is returned when no attribute is found.");

    const div = dom.parse("<div/>") as HTMLElement;
    dom.setAttr(div, "title", "something");
    assert.strictEqual(dom.getAttr(div, "title"), "something", "Set the title attribute.");
    assert.strictEqual(dom.getAttr(dom.parse("<div/>") as HTMLElement, "value"), null, "An unset value on a div returns undefined.");
    assert.strictEqual(dom.getAttr(dom.parse("<input/>") as HTMLElement, "value"), "", "An unset value on an input returns current value.");
...
export function setAttrTest() {
    ...
    assert.strictEqual(dom.getAttr(dom.parse("<input name=something>") as HTMLElement, "name"), "something", "Check element creation gets/sets the name attribute.");
    ...
    assert.strictEqual((document.getElementById("text1") as HTMLInputElement).readOnly, false, "Set readonly attribute");
    assert.strictEqual(dom.getAttr(document.getElementById("text1")!, "readOnly"), false, "Set readonly attribute");

    (document.getElementById("check2") as HTMLInputElement).checked = true;
    assert.strictEqual((document.getElementById("check2") as HTMLInputElement).checked, true, "Set checked attribute");
    assert.strictEqual(dom.getAttr(document.getElementById("check2")!, "checked"), true, "Set checked attribute");
    (document.getElementById("check2") as HTMLInputElement).checked = false;
    assert.strictEqual((document.getElementById("check2") as HTMLInputElement).checked, false, "Set checked attribute");
    assert.strictEqual(dom.getAttr(document.getElementById("check2")!, "checked"), false, "Set checked attribute");

    dom.setAttr(document.getElementById("check2")!, "checked", "checked");
    assert.strictEqual((document.getElementById("check2") as HTMLInputElement).checked, true, "Set checked attribute with 'checked'");
    assert.strictEqual(dom.getAttr(document.getElementById("check2")!, "checked"), true, "Set checked attribute");

    (document.getElementById("text1") as HTMLInputElement).readOnly = true;
    assert.strictEqual((document.getElementById("text1") as HTMLInputElement).readOnly, true, "Set readonly attribute");
    assert.strictEqual(dom.getAttr((document.getElementById("text1")!), "readOnly"), true, "Set readonly attribute");

    (document.getElementById("text1") as HTMLInputElement).readOnly = false;
    assert.strictEqual((document.getElementById("text1") as HTMLInputElement).readOnly, false, "Set readonly attribute");
    assert.strictEqual(dom.getAttr((document.getElementById("text1")!), "readOnly"), false, "Set readonly attribute");

    dom.setAttr(document.getElementById("name")!, "maxlength", "5");
    assert.strictEqual((document.getElementById("name") as HTMLInputElement).maxLength, 5, "Set maxlength attribute");
    dom.setAttr(document.getElementById("name")!, "maxLength", "10");
    assert.strictEqual((document.getElementById("name") as HTMLInputElement).maxLength, 10, "Set maxlength attribute");

    // HTML5 boolean attributes
    var $text = document.getElementById("text1") as HTMLInputElement;
    ...
    var td = dom.find(table, "td") as HTMLTableCellElement;
    ...
    var $radio = document.getElementById("testForm")!.appendChild(dom.parse("<input value='sup' type='radio'>")) as HTMLInputElement;
    ...
    var elem = dom.parse("<div style='position: absolute'></div>") as HTMLElement;
    dom.setAttr(elem, "style", null);
    assert.strictEqual(elem.style.cssText, "", "Check removing style attribute (#9699 Webkit)");

    var elem = dom.find("#fx-test-group") as HTMLElement;
    ...
    dom.setAttr(document.getElementById("check1")!, "checked", null);
    assert.strictEqual((document.getElementById("check1") as HTMLInputElement).checked, false, "removeAttr sets boolean properties to false");

    dom.setAttr(document.getElementById("text1")!, "readOnly", true);
    dom.setAttr(document.getElementById("text1")!, "readonly", null);
    assert.strictEqual((document.getElementById("text1") as HTMLInputElement).readOnly, false, "removeAttr sets boolean properties to false");
    ...
    var $first = dom.parse("<div Case='mixed'></div>") as HTMLElement;
    ...
    (document.getElementById("kk") as HTMLFormElement).reset();

    assert.strictEqual((document.getElementById("kkk") as HTMLInputElement).value, "cf", "Check value of select after form reset.");
...
export function getTextTest() {
    ...
    const val = "<div><b>Hello</b> cruel world!</div>";
    ...
    const check = document.getElementById("form")!.appendChild(dom.parse("<input type='checkbox' name='test' value='1'/>")) as HTMLElement;
    ...
    var button = dom.after(document.getElementById("button")!, "<button value='foobar'>text</button>") as HTMLElement;
...
export function setHtmlTest() {
    ...
    const div4 = dom.parse("<div />") as HTMLElement;
    dom.setHtml(div4, 5 as any);
    assert.strictEqual(dom.getHtml(div4), "5", "Setting a number as html");

    dom.setHtml(div, 0 as any);
    assert.strictEqual(dom.getHtml(div), "0", "Setting a zero as html");

    const div2 = dom.parse("<div/>") as HTMLElement, insert = "&lt;div&gt;hello1&lt;/div&gt;";
    ...
    const map = dom.parse("<map/>") as HTMLElement;
    ...
    var child = dom.query(document.getElementById("qunit-fixture")!, "script") as HTMLScriptElement[];
...
export function hasClassTest() {

    var x = dom.parse("<p>Hi</p>") as HTMLElement;
...
export function getStyleTest() {
    ...
    var div2 = dom.parse("<div style='display:none;'><input type='text' style='height:20px;'/><textarea style='height:20px;'></textarea><div style='height:20px;'></div></div>") as HTMLElement;
    ...
    "0 0.25 0.5 0.75 1".split(" ").forEach((n, i) => {
    ...
    eval("-[1,]") ?
        ...
        assert.ok(~(document.getElementById("empty") as any).currentStyle.filter.indexOf("gradient"), "Assert setting opacity doesn't overwrite other filters of the stylesheet in IE");
...
export function setStyleTest() {
    ...
    var div = document.getElementById("nothiddendiv")!,
        display = dom.getStyle(div, "display"),
        ret = dom.setStyle(div, "display", '');
    ...
    var div = document.getElementById("qunit-fixture")!.appendChild(dom.parse("<div>")) as HTMLElement;
...
export function computeStyleTest() {

}

export function getScrollTest() {

}

export function setScrollTest() {

}

export function getOffsetTest() {

}

export function setOffsetTest() {

}

export function offsetParentTest() {
...
export function setRectTest() {
    ...
    */
    ...
    var div = document.getElementById("foo") as HTMLElement;
    ...
    var div = document.body.appendChild(dom.parse("<div>")) as HTMLElement;
    div.style.position = "absolute";
    div.style.top = 0 as any;
    div.style.left = "10px";
    ...
    var p = document.getElementById("qunit-fixture")!.appendChild(dom.parse("<p>")) as HTMLElement;
...
export function animateTest() {

}

export function isHiddenTest() {
...
export function readyTest(done: Function) {
    dom.ready(() => {
