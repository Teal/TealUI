define(["require", "exports"], function (require, exports) {
    ...
    function parse(value, format) {
        if (format) {
            var groups_1 = [0];
            var obj_1 = {};
            var match = new RegExp(format.replace(/([-.*+?^${}()|[\]\/\\])/g, "\$1")
            ...
            if (match) {
                for (var i = 1; i < match.length; i++) {
                    obj_1[groups_1[i]] = +match[i];
                }
            }
            return new Date(obj_1.y || new Date().getFullYear(), obj_1.M ? obj_1.M - 1 : new Date().getMonth(), obj_1.d || 1, obj_1.H || 0, obj_1.m || 0, obj_1.s || 0);
        }
     ...
     */
    function getWeek(date, base) {
        if (base === void 0) { base = new Date(date.getFullYear(), 0, 1); }
     ...
     */
    function isValid(year, month, day, hour, minute, second, milliSecond) {
        if (day === void 0) { day = 1; }
