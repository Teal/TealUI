...
export default class Url {
     ...
     */
    private parse(url: string, parseQueryString?: boolean, slashesDenoteHost?: boolean) {
        ...
        if (!hostlessProtocol[proto as any] && (slashes || (proto && !slashedProtocol[proto]))) {
            ...
            // validate a little.
            if (!ipv6Hostname) {
                const r = validateHostname(this, rest, hostname);
                if (r !== undefined)
                    rest = r;
            }
    ...
    resolve(relative: string | Url) {
        ...
        for (const tkey in this as any) {
            (r as any)[tkey] = (this as any)[tkey];
        }
        ...
        const isSourceAbs = (r.pathname && r.pathname.charAt(0) === "/");
        const isRelAbs = (
            relative.host ||
        ...
        let srcPath = r.pathname && r.pathname.split("/") || [];
        const relPath = relative.pathname && relative.pathname.split("/") || [];
        const psychotic = r.protocol && !slashedProtocol[r.protocol];
        ...
        if (psychotic) {
            ...
            r.port = null!;
            if (r.host) {
                if (srcPath[0] === "") srcPath[0] = r.host;
                else srcPath.unshift(r.host);
            }
            r.host = "";
        ...
        if (isRelAbs) {
            ...
            if (psychotic) {
                ...
                const authInHost = r.host && r.host.indexOf("@") > 0 &&
                    r.host.split("@");
                if (authInHost) {
        ...
        const hasTrailingSlash = (
            (r.host || relative.host || srcPath.length > 1) &&
            (last === "." || last === "..") || last === "");
