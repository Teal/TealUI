...
function require(path, callback) {
    if (typeof path === "string") {
        const exports = require._ensure(require.toUrl(path), callback).exports;
...
require.getCurrentScript = () => {
    let currentScript = document.currentScript || require._currentScript;
    if (!currentScript) {
...
require.js = (path, callback) => {
    if (require._currentScript) {
        require._pendingJs.push({ path, callback });
        return;
    }
    ...
    script.onload = script.onreadystatechange = () => {
        ...
        if (readyState == undefined || readyState == "loaded" || readyState == "complete") {
            ...
            if (require._currentScript) {
                ...
                const js = require._pendingJs.shift();
                if (js) {
