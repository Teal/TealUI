...
1. [常量表达式(Constant Expression)](#3.1常量表达式)
2. [单目表达式(Unary Expression)](#3.2单目表达式)
3. [双目表达式(Binary Expression)](#3.3双目表达式)
4. [三目表达式(Conditional Expression)](#3.4三目表达式)
...
单目表达式即只有一个运算符的表达式。
单目运算符有：
- `+`(正)、`-`(负)、`~`([位反](../type.md#`~`(位反)：按位更新，原来的 1 改 0，原来的 0 改 1。)), 
- `!`(非), 
- `void`(空), 
- `delete`(删除)
- `typeof`(获取类型)
