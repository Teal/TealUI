...
2. `string`
3. `boolean`
4. `void`(在 JavaScript 称为 `undefined`)
5. `Array`
6. `Object`
7. `Function`
8. `RegExp`
...
其实所有类型都支持 `===`, `==`, `!==`, `!=` 这四个运算，具体将在[下一章 表达式](expression.md#)介绍，本章之后的章节将不再重复列出这四个运算。
...
##### 位与 `&`：按位合并，如果两个数字都是 1 则返回 1，否则返回 0。
...
##### `|`：按位合并，如果两个数字任一个是 1 则返回 1，否则返回 0。
...
2.10 类型转换
类型之间可以进行类型转换，在计算时自动进行的转换叫隐式类型转换。
明确写出的类型转换叫显示类型转换。

### 2.10.1 其它类型转到字符串类型
任何类型在转为字符串类型时，相当于调用了目标对象中，键为 `toString` 的成员函数。
默认地，每个类型转为字符串的方式如下：

- 数字转字符串：返回数字的十进制格式。如果数字过大或过小，则返回科学计数法格式。如 如 `1` 会转为 `"1"`； `-2.50000` 会转为 `"-2.5"`。
- 布尔转字符串：`true` 转为 `"true"`；`false` 转为 `"false"`。
- `undefined` 转字符串：转为 `"undefined"`。
- 数组转字符串：先把数组每个元素转为字符串，然后使用","连接。如 `[0, 1]` 会转为 "0,1"。
- 对象转字符串：`null` 转为 `"null"`；其它转为 `[object Object]`。
- 函数转字符串：转为函数的源码。
- 正则转字符串：转为正则的源码。
...
### 2.10.2 其它类型转到数字类型
任何类型在转为数字类型时，相当于调用了目标对象中，键为 `valueOf` 的成员函数。
默认地，每个类型转为字符串的方式如下：
...
- 数组转数字：先转为字符串，然后再按字符串转数字的规则转为数字。
- 对象转数字：`null` 转为 0；其它转为 `NaN`。
- 函数转数字：转为 `NaN`。
- 正则转数字：转为 `NaN`。
