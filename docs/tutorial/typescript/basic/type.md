...
在执行逻辑运算时，如果已经确定要返回前者时，后者是不回再继续计算的，这个现象称为懒计算。
...
调用函数时，可选参数可不用传值，系统会自动补上 `undefined`。
